...
pasta = r'G:\Drives compartilhados\arquitetura\Documentos\Arquitetura Operacional\BACKOFFICE FINANCEIRO\DASHBOARD\DASH_OPERACIONAL\LOOKER\Arquivos_diarios\BOLETOS_CELCOIN\E-mail'
...
for arquivo in os.listdir(pasta):
    # Se o arquivo terminar com '.xlsx' ou seja, for do tipo xlsx
...
df['data_Transacao'] = df['data_Transacao'].str.slice(0,10)
...
agrupamento_tipo = df.groupby(['data_Transacao','CreDeb','Classificacao2','Transacao','Fonte']).agg(soma=('valor','sum'),contagem=('valor','size')).reset_index().round(2)
